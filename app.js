...
function productCard(p){
  ...
  return `<article class="product-card">
    <a class="cover ${p.cover}" href="product.html?id=${p.id}" aria-label="${p.title}">
      <span class="product-badge">${p.badge||'منتج رقمي'}</span>
      <button class="wish-btn" type="button" aria-label="أضف للمفضلة" onclick="event.preventDefault();this.classList.toggle('active')">${icon('heart2')}</button>
      <span class="cover-brand">أثر</span>
...
function nav(){
  ...
  <header class="site-header"><div class="container nav-wrap">
    ...
    <div class="nav-actions">
      ...
      <button class="icon-btn" aria-label="المفضلة">${icon('heart2')}</button>
      <button class="icon-btn" aria-label="الحساب">${icon('user')}</button>
  ...
  </div><div class="mobile-nav"><a href="index.html">الرئيسية</a><a href="shop.html">المتجر</a><a href="about.html">عن أثر</a><a href="cart.html">السلة</a></div>
...
function footer(){ return `<footer class="footer"><div class="container footer-grid">
  ...
  <div><h4>أثر جديد في بريدك</h4><p>منتجات وأفكار وموارد مختارة، بدون إزعاج.</p><form class="newsletter" onsubmit="event.preventDefault();toast('شكرًا لك، تم الاشتراك بنجاح')"><input type="email" required placeholder="البريد الإلكتروني"><button>انضم</button></form></div>
...
function initHome(){
  const cats=$('#category-grid'); if(cats) cats.innerHTML=DATA.categories.map(c=>`<a class="category-card ${c.tone}" href="shop.html?category=${c.id}"><div class="category-icon">${icon(c.icon)}</div><div><span>${c.short}</span><h3>${c.name}</h3><p>${c.desc}</p><b>اكتشف القسم ${icon('arrow')}</b></div></a>`).join('');
  const featured=$('#featured-products'); if(featured) featured.innerHTML=DATA.products.filter(p=>p.featured).map(productCard).join('');
}
...
function initProduct(){
  const root=$('#product-page'); if(!root)return; const id=new URLSearchParams(location.search).get('id')||DATA.products[0].id; const p=DATA.products.find(x=>x.id===id)||DATA.products[0]; const cat=DATA.categories.find(c=>c.id===p.category);
  document.title=`${p.title} | أثر`;
  root.innerHTML=`<div class="product-detail-grid"><div class="product-gallery"><div class="cover detail-cover ${p.cover}"><span class="product-badge">${p.badge}</span><span class="cover-brand">أثر</span><strong>${p.title}</strong><small>${cat.short}</small><span class="cover-leaf">❧</span></div><div class="mini-previews"><span>معاينة 1</span><span>معاينة 2</span><span>معاينة 3</span></div></div>
  <div class="product-info"><a class="crumb" href="shop.html?category=${cat.id}">${cat.name}</a><h1>${p.title}</h1><div class="rating big"><span>★</span> ${p.rating} <em>(${p.reviews} تقييم)</em></div><p class="lead">${p.subtitle}</p><div class="price-big">${money(p.price)} ${p.oldPrice?`<del>${money(p.oldPrice)}</del>`:''}</div><div class="buy-actions"><button class="primary-btn" onclick="addToCart('${p.id}')">${icon('bag')} أضف للسلة</button><button class="secondary-btn" onclick="addToCart('${p.id}');location.href='cart.html'">اشترِ الآن</button></div>
  ...
  <div class="product-spec"><div><span>النوع</span><b>${p.format}</b></div><div><span>المحتوى</span><b>${p.pages}</b></div><div><span>الوصول</span><b>فوري بعد الدفع</b></div></div></div></div>
  <section class="detail-copy"><h2>هذا المنتج سيساعدك على</h2><div class="benefit-list"><p>✓ تحويل الفكرة إلى خطوات بسيطة وقابلة للتطبيق.</p><p>✓ المتابعة بدون تعقيد أو شعور بالضغط.</p><p>✓ بناء وعي أعمق بالموضوع بطريقة عملية.</p><p>✓ الاحتفاظ بنسختك والرجوع لها في أي وقت.</p></div><h2>ماذا ستحصل عليه؟</h2><p>نسخة رقمية مصممة بعناية، جاهزة للتحميل والاستخدام فور إتمام عملية الشراء. يمكن استخدامها على الهاتف أو الكمبيوتر، وطباعتها عندما يكون المنتج قابلًا للطباعة.</p></section>`;
...
function renderCart(){
  const root=$('#cart-root'); if(!root)return; const cart=getCart(); const rows=cart.map(row=>({row,p:DATA.products.find(p=>p.id===row.id)})).filter(x=>x.p);
...
document.addEventListener('DOMContentLoaded',()=>{initGlobal();initHome();initShop();initProduct();renderCart();});
